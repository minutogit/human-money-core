...
mod internal_tests {
    ...
    fn make_fp(t_id: &str, encrypted_timestamp: u128) -> TransactionFingerprint {
        TransactionFingerprint {
            ds_tag: "tagX".to_string(),
            u: "none".to_string(),
    ...
    fn test_resolve_conflict_offline_earliest_wins() {
        ...
        // Wir setzen die Nanos manuell über die Entschlüsselungsfunktion voraus.
        // encrypt_transaction_timestamp leitet den Schlüssel aus t_id und prev_hash ab.
        // Um das deterministisch zu testen, verwenden wir direkt die Logik oder berechnen es.
        // Einfacher: Wir mocken die Transaktionen so, dass decrypt_transaction_timestamp
        // bestimmte Nanos zurückliefert.
        // Aber decrypt_timestamp ist eine echte Funktion!
        // Wir erzeugen echte verschlüsselte Timestamps.
        ...
        let fp_early = make_fp(&t_id_early, enc_early);
        let fp_late = make_fp(&t_id_late, enc_late);
        
        resolve_conflict_offline(&mut store, &[fp_early, fp_late]);
        
        // Der frühere Timestamp muss Active bleiben, der spätere Quarantined
