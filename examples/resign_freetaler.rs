...
fn main() {
    ...
    let (public_key, signing_key) = crypto::generate_ed25519_keypair_for_tests(Some(mnemonic));
    let issuer_id = crypto::create_user_id(&public_key, Some("issuer")).unwrap();
    ...
    let hash = crypto::get_hash(canonical_json.as_bytes());
    
    // Sign
    let signature = crypto::sign_ed25519(&signing_key, hash.as_bytes());
