...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    service_alice.create_profile("Alice", &AppService::generate_mnemonic(12)?, None, Some("alice"), password)?;
    ...
    service_bob.create_profile("Bob", &AppService::generate_mnemonic(12)?, None, Some("bob"), password)?;
    ...
    let (std_def, _) = human_money_core::services::standard_manager::verify_and_parse_standard(&standard_toml)?;
...

    {
        ...
        let profile = profiles.iter().find(|p| p.profile_name == "Creator").expect("Creator profile missing");
        ...
        let voucher = service_creator.create_new_voucher(&standard_toml, "de", voucher_data, None)?;
    ...
    let tx1_json;
    {
        ...
        let bundle_result = service_creator.create_transfer_bundle(request, &standards_map, None, Some(password))?;
        
        // Alice receives
        service_alice.unlock_session(password, 60)?;
        let _ = service_alice.receive_bundle(&bundle_result.bundle_bytes, &standards_map, None, Some(password))?;
    ...
    let tx2_json;
    {
        ...
        let summaries = service_creator.get_voucher_summaries(None, None)?;
        println!("   Creator wallet has {} vouchers (should be 1).", summaries.len());
        ...
        // Bob receives
        service_bob.unlock_session(password, 60)?;
        let _ = service_bob.receive_bundle(&bundle_result.bundle_bytes, &standards_map, None, Some(password))?;
    ...
    if ds_tag1 == ds_tag2 {
        ...
        println!("   Even though the amounts (40.00 vs 99.00) and recipients (Alice vs Bob) are different,");
