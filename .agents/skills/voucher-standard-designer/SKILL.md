...
**Typische CEL-Beispiele:**
```toml
[immutable.custom_rules.max_transfer_amount]
expression = "Transaction.amount <= 5000"
message = "Ein einzelner Transfer darf 5000 Einheiten nicht überschreiten."
...
message = "Der Aussteller darf nicht als eigener Bürge auftreten."
```
...
## 4. Instruktionen für die KI bei Benutzeranfragen

Wenn ein Benutzer einen Gutschein-Standard entwerfen möchte:
1. **Analysiere den Einsatzzweck:** Frage kurz nach dem Verwendungszweck (z. B. Nachbarschaftshilfe, Firmennetzwerk, Sachwert, Event), falls unklar.
2. **Wähle den passenden Archetyp:** Verwende einen der 4 Archetypen als Ausgangsbasis.
3. **Generiere eine vollständige, fehlerfreie `standard.toml`:**
   * Gib den TOML-Code in einem sauberen ` ```toml ` Markdown-Codeblock aus.
   * Verwende stets eine neue, gültige UUID v4 für `[immutable.identity.uuid]`.
   * Setze alle Enums exakt wie spezifiziert (`goods_or_services`, `personal_guarantee`, etc.).
   * Achte auf gültige ISO 8601 Zeitangaben (`P1Y`, `P5Y`, `P6M`).
   * Schreibe aussagekräftige i18n-Texte mit `{{amount}}` in `de` und `en`.
4. **Hinweis für den Nutzer:** Weise den Nutzer darauf hin, dass er die generierte `standard.toml` einfach kopieren und in die **AI Fast-Lane** des Voucher Standard Builders einfügen kann, um sie in Sekundenschnelle per WASM zu validieren und digital zu signieren.
