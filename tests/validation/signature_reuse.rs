...
use human_money_core::test_utils::{
    ACTORS, MINUTO_STANDARD as BASE_STANDARD, create_minuto_voucher_data, create_guarantor_signature,
};
...
fn test_prevent_signature_reuse_in_init() {
    ...
    voucher1.signatures.push(create_guarantor_signature(&voucher1, g1, "G1", "guarantor", "1"));
    voucher1.signatures.push(create_guarantor_signature(&voucher1, g2, "G2", "guarantor", "2"));
    
    // Validate voucher1 - this must succeed
    assert!(validate_voucher_against_standard(&voucher1, &standard).is_ok(), "Voucher 1 must be valid");
    ...
    let identity_sig_bytes = human_money_core::services::crypto_utils::sign_ed25519(&identity.signing_key, &t_id_raw);
    bad_tx.sender_identity_signature = Some(bs58::encode(identity_sig_bytes.to_bytes()).into_string());
    ...
    let (genesis_secret, genesis_pub) = human_money_core::services::crypto_utils::derive_ephemeral_key_pair(
        &identity.signing_key, &nonce_bytes, "genesis", Some(creator_prefix)
    ).unwrap();
    ...
    let v_id = human_money_core::services::l2_gateway::calculate_layer2_voucher_id(&bad_tx).unwrap();
    ...
    if let Err(e) = &result {
        println!("Validation error for Voucher 2 (expected success if vulnerable): {:?}", e);
    }
