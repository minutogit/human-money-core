...
fn test_scenario_1_happy_path() {
    ...
    let (mut app, _) = test_utils::setup_service_with_profile(dir.path(), &ACTORS.test_user, "Alice", correct_password);
    ...
    app.create_new_voucher(&flexible_toml, "en", NewVoucherData {
...
fn test_scenario_2_offline_sync() {
    ...
    // Tx 1
    app.create_transfer_bundle(human_money_core::wallet::MultiTransferRequest {
        recipient_id: id_david.clone(),
        sources: vec![human_money_core::wallet::SourceTransfer { local_instance_id: voucher_id.clone(), amount_to_send: "10".to_string() }],
        notes: None, sender_profile_name: None,
    }, &standards_toml, None, Some(correct_password)).unwrap();
    
    let v_id_tx1 = app.get_voucher_summaries(None, Some(&[human_money_core::wallet::instance::VoucherStatus::Active])).unwrap().last().unwrap().local_instance_id.clone();

    // Tx 2
    app.create_transfer_bundle(human_money_core::wallet::MultiTransferRequest {
        recipient_id: id_david.clone(),
        sources: vec![human_money_core::wallet::SourceTransfer { local_instance_id: v_id_tx1.clone(), amount_to_send: "5".to_string() }],
        notes: None, sender_profile_name: None,
    }, &standards_toml, None, Some(correct_password)).unwrap();
    
    let v_id_tx2 = app.get_voucher_summaries(None, Some(&[human_money_core::wallet::instance::VoucherStatus::Active])).unwrap().last().unwrap().local_instance_id.clone();
    ...
    // Process output
    app.process_l2_response(&v_id_tx2, &resp_query, Some(correct_password)).unwrap();
}
...
fn test_scenario_3_double_spend_detection() {
    ...
    let v_id_transfer = app.get_voucher_summaries(None, Some(&[human_money_core::wallet::instance::VoucherStatus::Active])).unwrap().last().unwrap().local_instance_id.clone();
    ...
    app.process_l2_response(&v_id_transfer, &resp_query, Some(correct_password)).unwrap();

    // 5. Check if quarantined
    let details = app.get_voucher_details(&v_id_transfer).unwrap();
    assert!(matches!(details.status, human_money_core::wallet::instance::VoucherStatus::Quarantined { .. }));
}
