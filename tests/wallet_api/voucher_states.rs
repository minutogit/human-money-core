...
fn test_quarantined_voucher_behavior() {
    ...
    let signing_request_result = wallet.create_signing_request(alice, &local_id, ContainerConfig::TargetDid(ACTORS.guarantor1.user_id.clone(), PrivacyMode::TrialDecryption));
    assert!(
        matches!(
