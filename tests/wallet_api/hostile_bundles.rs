...
fn test_rejection_of_voucher_replay_in_new_bundle() {
    ...
    let voucher_a = service_sender.create_new_voucher(&silver_toml, "en", NewVoucherData { creator_profile: PublicProfile { id: Some(service_sender.get_user_id().unwrap()), ..Default::default() }, nominal_value: ValueDefinition { amount: "100".to_string(), ..Default::default() }, ..Default::default() }, "pwd").unwrap();
