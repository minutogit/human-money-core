...
fn test_signature_bypass_mechanism() {
    ...
    assert!(result_fail.is_err(), "Validierung sollte fehlschlagen, da Signatur defekt ist!");
    ...
    assert!(result_ok.is_ok(), "Validierung sollte trotz defekter Signatur erfolgreich sein, da Bypass aktiv ist! Fehler war: {:?}", result_ok.err());
    ...
    assert!(result_fail_again.is_err(), "Bypass sollte wieder inaktiv sein!");
...
fn test_logic_modification_with_bypass() {
    ...
    if let Err(e) = &result {
        ...
        assert!(!err_msg.contains("Signature"), "Fehler darf kein Signaturfehler sein! Erhalten: {}", err_msg);
    }
}
