...
use human_money_core::error::{VoucherCoreError, ValidationError};
use human_money_core::to_canonical_json;

// --- Helper: Create voucher with specific privacy mode ---
fn create_privacy_test_voucher(mode: &str) -> (Voucher, human_money_core::models::voucher_standard_definition::VoucherStandardDefinition) {
...
fn test_privacy_mode_public_fails_missing_sender() {
    ...
    let result = voucher_validation::validate_voucher_against_standard(&voucher, &standard);
    assert!(matches!(result, Err(VoucherCoreError::Validation(ValidationError::InvalidTransaction(msg))) if msg.contains("missing sender_id")), "Should fail due to missing sender_id in public mode");
}
...
fn test_privacy_mode_public_fails_anonymous_recipient() {
    ...
    let result = voucher_validation::validate_voucher_against_standard(&voucher, &standard);
    assert!(matches!(result, Err(VoucherCoreError::Validation(ValidationError::InvalidTransaction(msg))) if msg.contains("non-DID recipient")), "Should fail due to non-DID recipient in public mode");
}
...
fn test_privacy_mode_stealth_fails_with_sender_id() {
    ...
    let result = voucher_validation::validate_voucher_against_standard(&voucher, &standard);
    assert!(matches!(result, Err(VoucherCoreError::Validation(ValidationError::InvalidTransaction(msg))) if msg.contains("has sender_id")), "Should fail if sender_id is present in stealth mode");
}
...
fn test_privacy_mode_stealth_fails_with_did_recipient() {
    ...
    let result = voucher_validation::validate_voucher_against_standard(&voucher, &standard);
    assert!(matches!(result, Err(VoucherCoreError::Validation(ValidationError::InvalidTransaction(msg))) if msg.contains("public DID recipient")), "Should fail if recipient is DID in stealth mode");
}
