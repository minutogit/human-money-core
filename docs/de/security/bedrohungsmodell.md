...
| :--- | :--- | :--- | :--- | :--- |
| **S**poofing | **Identitätsverschleierung** | Ein Angreifer gibt vor, der Inhaber eines Vouchers zu sein (Replay Attacke). | **TrapData Context Binding**: Jeder Voucher-Transfer ist an den Hash der Vorgänger-Transaktion gebunden. Die Identität wird durch Signaturen (Public Mode) oder ZKP (Stealth Mode) bewiesen. | ✅ Implementiert |
...
| **R**epudiation | **Abstreitbarkeit** | Ein Nutzer leugnet, eine Zahlung getätigt zu haben. | **Signaturen**: Transaktionen (im Public Mode) sind signiert. Im Stealth-Mode ist dies *by design* möglich (Feature, nicht Bug), aber Double-Spending wird durch TrapData verhindert. | ⚠️ By Design (Privacy) |
| **I**nformation Disclosure | **Informationsleck** | Metadaten (Zeitstempel, IDs) lecken in Stealth-Transaktionen. | **Privacy Matrix**: Validation Logic verbietet strikt das Vorhandensein von IDs im Stealth Mode. (Siehe `datenschutz_analyse.md`). | 🔄 In Arbeit (Härtung) |
...
1.  **Stealth Mode Implementation**: Die Gefahr, dass Validierungsregeln (wie TrapData-Checks) übersprungen werden, weil Felder fehlen ("Fail Open" statt "Fail Close").
