...
**Triage:** `[CONFIRMED VULNERABILITY]` in the specification. No minimal src-fix is possible without inventing the entire (future) trust-verifier feature — an architectural change out of audit scope.

**Action:** Conformance harness committed as ignored test documenting (a) a working spec-conformant verifier, (b) the concrete forgery, and (c) the secure invariant the future implementation MUST satisfy (bind issuer signature to canonical form of all semantic fields, excluding only `issuer_signature`). **The protocol document must be fixed BEFORE any verifier is implemented**, or every issued assertion becomes invalid upon remediation.
...
**Triage:** `[CONFIRMED VULNERABILITY]`. (1) Oracle lives on the public envelope; (2) offline forensics unaffected (SA06-05 event logs carry attribution); (3) SA06-01 remediation already established inner-bundle-signature authority and unsigned envelopes as legitimate; (4) receive path accepts empty signatures, so nothing breaks.
...
| Severity | High |
| CWE | CWE-347 |
| Target | `app_service/app_signature_handler.rs::open_voucher_signing_request`; `wallet/signature_handler.rs::process_and_attach_signature` |
...
**Regression verification:** `cargo nextest run -E 'test(api_wallet_signature)'` → 6 passed (foreign suite exercising these paths, incl. AEAD-precedence contract).
...
| Severity | Medium |
| CWE | CWE-74 / CWE-1164 |
| Target | `wallet/transaction_handler.rs::process_encrypted_transaction_bundle_inner` (`counterparty_name ← bundle.sender_profile_name` → `EventBffData`, `BundleMetadataStore.history`) |
...
**Triage:** `[CONFIRMED VULNERABILITY]`. Distinct from protected SA06-05 design: sanitizing the display NAME does not touch the forensic anchor (`counterparty_id`); retention asserted intact inside the test.

**Regression verification:** `cargo nextest run -E 'test(event)'` → 18 passed.
